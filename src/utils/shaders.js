...
void main() {
  ...
  vec2 mouseWolrdCoord = mouseDistortedUV * aspectRatio;

  mouseWolrdCoord *= uZoom;
  mouseWolrdCoord += uOffset;

  vec2 mouseCellPos = mouseWolrdCoord / uCellSize;
  ...
  float hoverIntensity = 1.0 - smoothstep(o.4,0.7,cellDistance);
  float isHovered = hoverIntensity > 0.0 && uMousePos.x >= 0.0;
  ...
  float gridX = smoothstep(0.0,lineWidth,cellUV.x) * smoothstep(0.0,lineWidth,1.0-cellCenter);
  float gridY = smoothstep(0.0,lineWidth,cellUV.y) * smoothstep(0.0, lineWidth,1.0-cellCenter);
  float gridMask = gridX * gridY;

  float imageSize = 0.6;
  float imageBorder = (1.0 - imageBorder) / imageSize;
  ...
  bool imImageArea = imageUV.x >= 0.0 && imageUV.x <= 1.0 && imageUV.y >= 0.0 && imageUV.y <= 1.0;
